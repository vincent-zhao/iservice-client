...
exports.create = function (options) {
  ...
  _me.get = function (key, callback) {
    ...
    var fna = Tool.normalize(Util.format('%s/config_cache/%d/%s/%s.zk', _options.cache, process.pid, _options.root, key));
    ...
    _http.get(_buildURL(root, 'tree'), function (error, data) {
      ...
      _parseRes(data, function (error, data) {
        ...
        var dir = Tool.clean(Tool.normalize(Util.format(
              '/%s/config_cache/%d/%s/%s', _options.cache, process.pid, _options.root, root)), 2);
