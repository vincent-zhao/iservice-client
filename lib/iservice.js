...
exports.create = function (options) {
  ...
  _me.get = function (key, callback) {
    key = normalize('/' + key);
    if (_caches[key]) {
      callback(null, _caches[key].data, _caches[key].meta);
    } else {
      callback(iError('NotFound', Util.format('node "%s" not found.', key)));
    }
  };
  /* }}} */
  ...
  _me.sync  = function (root, callback) {
    _http.get(_buildURL(root, 'tree'), function (error, data) {
      ...
      fs.writeFile(fname + '.zk', JSON.stringify(data), 'utf-8', function (error) {
        if (!error) {
          _caches = data;
        }
        callback(error);
