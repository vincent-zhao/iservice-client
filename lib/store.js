...
Storage.prototype.sync = function (key, callback) {
  ...
  var _sync = function (root) {
    _self._handle.a_get_children(root, null, function (code, error, children) {
      if (0 !== code) {
      ...
      _self._handle.a_get(root, null, function (code, error, stat, data) {
        if (0 !== code) {
          return _fail(iError('ZookeeperError', error));
        }

        try {
          _self._backup(root, data);
        } catch (e) {
          return _fail(iError('BackupError', e));
        }
        if (0 === (--_wait)) {
          callback(null);
        }
