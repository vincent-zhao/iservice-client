...
Subscribe.prototype.set = function () {
  var _self = this;
  var tree = _self.store.getTree(_self.root);
  var map = {};
  for (var key in tree) {
    try {
  ...
  process.nextTick(function(){
    ...
    if (newServices.length === 0) {
      _self.emit('error', _self.name + ' has no machine');
    } else {
    ...
    _self.allServices.forEach(function (service) {
      func(service, function (error) {
        ...
        if (--count === 0) {
          ...
          _self.emit('HBCheck', Tool.clone(_self.availables));
...
/*{{{ defaultHttp() */
function defaultHttp(obj, callback, options){
  var _options = {
    host : obj.host,
    port : obj.port,
